...
def catalog():
    ...
    recentItems = db_session.query(Item).order_by(
        Item.created.desc()).limit(5).all()

    return render_template(
        'catalog.html', user=user,
...
def gdisconnect():
    # Disconnect a user from session.
...
def newItem():
    ...
    if request.method == 'GET':
        ...
        return render_template(
            'new_item.html', categories=categories, user=user)
    else:
